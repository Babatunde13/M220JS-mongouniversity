...
export default class MoviesDAO {
  ...
  static async getMovies({
    ...
    */
    ...
    let moviesToSkip = page * moviesPerPage
    const displayCursor = cursor.limit(moviesPerPage).skip(moviesToSkip)

    try {
      const moviesList = await displayCursor.toArray()
      let totalNumMovies = await movies.countDocuments(query)
      totalNumMovies = page === 0 ? totalNumMovies : totalNumMovies - moviesToSkip
